...
public:
   ...
   {item_color=system_settings_spider::COLOR_BLACK;};
    ...
    int get_device_type_id() const { return device_type_id;}
    int get_command_code() const { return command_code;}
    int get_device_number() const {return device_number; }
	int get_station() const {	return station_id;}
    string get_command_description()  const  { return command_description;};
    void set_item_color(const PgColor_t& color_of_item){item_color=color_of_item;}
    PgColor_t get_item_color()  const {return  item_color;}
