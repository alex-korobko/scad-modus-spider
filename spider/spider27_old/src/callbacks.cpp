...
extern metro_devices_container g_devices;
...
extern int g_chanID;
extern pthread_t g_pingTID;
extern pthread_t g_timerTID;


log_filter temp_log_filter;
contain_timer_command tmp_morning_start;
	...
	if (sel_item_count>0) 
	{
		...
		if (ptr_to_cmd_pool->size()>static_cast<unsigned short>(sel_indexes[0]-1))
		{
			...
			advance(iter_cmd_pool, 
							sel_indexes[0]-1);			
			iter_cmd_pool->send(&g_escalators);
...
RealizeMainWnd( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	//setting beginning state of filtration log items and visualize it
	...
	//setting beginning state of morning start and visualize it
	{
		string indicator_text;
		vector<PtArg_t> args(3);
		
		g_lines.set_morning_start_state_indicator(ABW_esc_config_btn);
		PtSetArg( &args[0], Pt_ARG_LABEL_IMAGE, g_system_settings.get_image(system_settings::BLOCK_GREY_LED), 0);
		PtSetArg( &args[1], Pt_ARG_COLOR, system_settings::COLOR_BLACK, 0);				
		indicator_text="Линий 0 стартов 0";

		PtSetArg( &args[2], Pt_ARG_TEXT_STRING, indicator_text.c_str(),  indicator_text.size());
		PtSetResources (ABW_esc_config_btn, args.size(), &args[0]);	
	}
...
ResizeScheme( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	PtArg_t		arg;
	PhDim_t		*wnd_size;
	PtContainerCallback_t	*data = static_cast<PtContainerCallback_t*> (cbinfo->cbdata);
	vector<PhPoint_t>  points;
...
//					temp_metro_stations_iterator->second.set_y_coord(temp_point.y);
					...
					if (temp_metro_stations_iterator->second.get_wnd()!=NULL)
						{		
							...
							temp_point.x = temp_metro_stations_iterator->second.get_x_coord()*data->new_dim.w/100 - wnd_size->w/2;
							temp_point.y = temp_metro_stations_iterator->second.get_y_coord()*data->new_dim.h/100 - wnd_size->h/2;
...
RealizeEscalatorPanel( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	metro_stations_container::iterator_metro_stations station_iterator;
		...
		{	
			...
			metro_devices_types_container::iterator_devices_types iter_types;
...
PopupControlMenu( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	PtWidget_t *ptr_to_instance_wnd=ApGetInstance(widget);
	if (ptr_to_instance_wnd==NULL) 
		{
			g_system_settings.sys_message(system_settings::ERROR_MSG, "ptr_to_instance_wnd==NULL");
			return( Pt_CONTINUE ); // that escalator not found
		};

	PtWidget_t *ptr_to_esc_wnd=ApGetWidgetPtr(widget, ABN_escalator_wnd);

	if (ptr_to_esc_wnd==NULL) 
		{
			g_system_settings.sys_message(system_settings::ERROR_MSG, "ptr_to_esc_wnd==NULL");
			return( Pt_CONTINUE ); // that escalator not found
		};

	PtGetResource(ptr_to_esc_wnd, Pt_ARG_USER_DATA, &escalator_id, 0);
	esc_iter=g_escalators.find(escalator_id);	
	if (esc_iter==g_escalators.end())  
		{
			vector<char> tmp_chars(10);
			itoa(escalator_id, &tmp_chars[0], 10);
			string mess="PopupControlMenu : not found esc id ";
			mess+=&tmp_chars[0];
			g_system_settings.sys_message(system_settings::ERROR_MSG, mess);
			return( Pt_CONTINUE ); // that escalator not found
		}
...
item_selection_cb_lines( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	metro_lines_container::iterator_metro_lines line_iter=g_lines.begin();
	advance(line_iter, static_cast<int>(sel_item_index-1));
...
activate_esc_conf_save( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	{
	if (g_lines.empty()) return( Pt_CONTINUE);
	bool new_morn_start_equal_to_old;
	//setting new morning start
	PtWidget_t *ptr_to_EscConfig = ApGetInstance(widget);
	contain_morning_start *ptr_morn_start_cont=NULL;	
	...
	line_iter->second.morning_start.sort_all();
	ptr_morn_start_cont->sort_all();
	new_morn_start_equal_to_old=(line_iter->second.morning_start==*ptr_morn_start_cont);
	//setting to escalators new parameters
	if (! new_morn_start_equal_to_old)
		{
			line_iter->second.morning_start=*ptr_morn_start_cont;
			
		};

	//setting beginning state of morning start and visualize it
	{
		int lines_count_with_prepared_morn_start=0, morn_start_count=0;
		string indicator_text;
		vector<PtArg_t> args(3);
		...
		while (iter_lines!=g_lines.end())
			{
			if (iter_lines->second.morning_start.get_was_created_in_this_day())
				{
					lines_count_with_prepared_morn_start++;
					if (! iter_lines->second.morning_start.only_started_mode_enabled())
								iter_lines->second.morning_start.only_starting();

					morn_start_count+=iter_lines->second.morning_start.size();
				};

			iter_lines++;
			};	

		if (lines_count_with_prepared_morn_start>0)
			{
				PtSetArg( &args[0], Pt_ARG_LABEL_IMAGE, g_system_settings.get_image(system_settings::BLOCK_GREEN_LED), 0);
				PtSetArg( &args[1], Pt_ARG_COLOR, system_settings::COLOR_GREEN, 0);				
				indicator_text="Линий ";
				vector<char> tmp_chars(10);
				itoa( lines_count_with_prepared_morn_start, 
						&tmp_chars[0],
						10);
				indicator_text+=&tmp_chars[0];
				indicator_text+=" стартов ";
				itoa( morn_start_count, 
						&tmp_chars[0],
						10);
				indicator_text+=&tmp_chars[0];
			} else {
		...
		PtSetArg( &args[2], Pt_ARG_TEXT_STRING, indicator_text.c_str(),  indicator_text.size());
		PtSetResources (g_lines.get_morning_start_state_indicator(), args.size(), &args[0]);	
	}
