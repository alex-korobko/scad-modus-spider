...
int ChangeRoute(in_addr dest, in_addr gateway, int flags, int cmd)
{
	...
	string  tmp_str("change_route dest ");
	tmp_str+=inet_ntoa(dest);
	tmp_str+=" gateway ";
	tmp_str+=inet_ntoa(gateway);
	tmp_str+=(cmd==router::ADD_ROUTE)? " add" : " delete";
	
	g_system_settings.sys_message(system_settings::ERROR_MSG, tmp_str);
